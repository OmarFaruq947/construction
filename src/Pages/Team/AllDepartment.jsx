import React from "react";

const AllDepartment = () => {
  return (
    <>
      <div className="flex flex-col text-center w-full mb-10 pt-14">
        <h2 className="text-secondary font-bold text-4xl uppercase relative h-auto w-full pb-1 text-center ">
          Company
          <span className="text-brand"> Departments</span>
        </h2>
        <p className="lg:w-2/3 mx-auto leading-relaxed text-textColure">
          It's important to note that the specific departments in a construction
          company may vary depending on the size, specialization, and
          organizational structure of the company. Some companies may combine
          certain functions or have additional departments based on their
          specific needs and operations.
        </p>
      </div>

      <div class="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
        <div class="grid row-gap-8 sm:row-gap-0 sm:grid-cols-2 lg:grid-cols-3">
          <div class="p-8 border-b sm:border-r border-brand  transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center ">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Engineering Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Sed ut perspiciatis unde omnis iste natus error sit voluptatem
                accusantium rem aperiam, eaque ipsa quae. Sit error voluptatem.
              </p>
            </div>
          </div>

          <div class="p-8 border-b lg:border-r border-brand transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Procurement Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Storage shed, troughs feed bale manure, is garden wheat oats at
                augers. Bulls at rose garden cucumbers mice sunflower wheat in
                pig.
              </p>
            </div>
          </div>

          <div class="p-8 border-b sm:border-r lg:border-r-0 border-brand transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Human Resources Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Sed ut perspiciatis unde omnis iste natus error sit voluptatem
                accusantium rem aperiam, eaque ipsa quae. Sit error voluptatem.
              </p>
            </div>
          </div>

          <div class="p-8 border-b lg:border-b-0 lg:border-r border-brand transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Quality Assurance/Quality Control Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Storage shed, troughs feed bale manure, is garden wheat oats at
                augers. Bulls at rose garden cucumbers mice sunflower wheat in
                pig.
              </p>
            </div>
          </div>

          <div class="p-8 border-b sm:border-b-0 sm:border-r border-brand transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Information Technology (IT) Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Sed ut perspiciatis unde omnis iste natus error sit voluptatem
                accusantium rem aperiam, eaque ipsa quae. Sit error voluptatem.
              </p>
            </div>
          </div>

          <div class="p-8 transition duration-400 hover:bg-brand">
            <div class="max-w-md text-center">
              <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-brand sm:w-16 sm:h-16">
                <svg
                  class="w-8 h-8 text-white sm:w-12 sm:h-12"
                  stroke="currentColor"
                  viewBox="0 0 52 52"
                >
                  <polygon
                    stroke-width="3"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    fill="none"
                    points="29 13 14 29 25 29 23 39 38 23 27 23"
                  ></polygon>
                </svg>
              </div>
              <h6 class="mb-2 font-semibold leading-5">
                Finance and Accounting Department
              </h6>
              <p class="mb-3 text-sm text-gray-900">
                Storage shed, troughs feed bale manure, is garden wheat oats at
                augers. Bulls at rose garden cucumbers mice sunflower wheat in
                pig.
              </p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default AllDepartment;
